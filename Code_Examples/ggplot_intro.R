...
options(scipen = 999) #NO scientific notation on graphs unless they are really big numbers
...
ggplot(midwest, aes(x=area, y=poptotal)) + 
  geom_point() + 
  geom_smooth(method ="lm", linetype= 2, color="turquoise")
...
p + facet_wrap(~state,scales="free") #giving freedom to each stae graph to make it's own scale according to the data involved

# Store this new zoomed-in plot
p2 <- p + coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000))
 #gg is the Gramar of Graphics
...
ggplot(midwest, aes(x=area, y=poptotal)) + #global aesthetics for the rest of the lines, this will be inherited for the rest of the set
  geom_point() + #inherited the global aesthetics from the X and Y axis stated above
  geom_smooth(method="lm") + #running a linear regression in the background, a lowes curve
  coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) + #lim means that you don't want it to delete any of the points after creating the linear regression line
...
ggplot(midwest, aes(x=area, y=poptotal)) + 
  geom_point(color="steelblue",size=3,alpha=.5) + 
  geom_smooth(method="lm",color="firebrick",se=FALSE,linetype=2)+ 
...
p3 + scale_color_brewer(palette = "BrBG")
mypallette <- c("#4287f5","#41d0f0","#4175f0","#6f41f0","#b041f0")
...
p4 = ggplot(midwest, aes(x=area/max(area), y=log10(poptotal))) + 
  geom_point(aes(color=state),size=1,alpha=.5) + 
  geom_smooth(method="lm",color="firebrick") + 
...
ggsave("./test.png",width=8,height = 6,dpi=300)#this will save the last graph that was opened to your device
